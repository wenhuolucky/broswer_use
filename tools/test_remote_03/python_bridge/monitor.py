...
支持多用户模式：--user user1/user2/user3 自动映射端口
"""
import asyncio
import json
import sys
...
# 将 test_remote_03 加入搜索路径，保证 import python_bridge.xxx 可用
...
# 用户端口映射表（与 users.yaml / docker-compose.yml 一致）
USER_PORTS = {
    "user1": {"cdp": 9227, "novnc": 6081},
    "user2": {"cdp": 9237, "novnc": 6082},
    "user3": {"cdp": 9247, "novnc": 6083},
}
...
async def main():
    ...
    parser.add_argument("--user", choices=list(USER_PORTS.keys()), default=None,
                        help="用户 ID（自动映射 CDP 端口和输出路径）")
    parser.add_argument("--cdp-url", default=None, help="Chrome CDP 地址（指定 --user 时可省略）")
    ...
    parser.add_argument("--output", default=None, help="Cookie 输出路径（指定 --user 时可省略）")
    ...
    if args.user:
        ports = USER_PORTS[args.user]
        if cdp_url is None:
            cdp_url = f"http://localhost:{ports['cdp']}"
        if output_path is None:
            output_path = f"output/{args.user}/cookies.json"
        print(f"[用户] {args.user} → CDP:{ports['cdp']}, noVNC:{ports['novnc']}")

    if cdp_url is None:
        cdp_url = "http://localhost:9222"
    if output_path is None:
        output_path = "output/cookies.json"
