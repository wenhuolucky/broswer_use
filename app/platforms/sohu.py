...
"""
    ...
    def _sohu_cover_rules(cover_instruction: str) -> str:
        """搜狐号封面指导（2026-06-10 简化版）：始终走"正文图片 + 第一张"。

        旧版区分"有本地路径走本地上传、无本地路径走正文图片"，但实际
        联调发现"本地上传"总会卡"图片上传中"伪状态，需要 fallback 到
        素材库，路径曲折。改为统一走"正文图片" + click 第一张，
        省略本地上传和素材库 fallback（仅在正文无图时兜底）。
        """
        return """
...
- 兜底：仅在"正文图片"列表为空（正文里没图）时，才切到"素材库"选列表第一张图片；都失败才停止。
- **禁止**走"本地上传"——即使 cover_instruction 给出了本地路径，也忽略，避免"图片上传中"伪状态卡死。
- 任何情况下都必须保持**单封面**，不要选三封面或五封面。
- 上面 cover_instruction 中"本地上传"、"素材库 fallback"、"file input 上传"等 Toutiao 默认文案**不适用搜狐号**，必须按本节走"正文图片 + 第一张"。
"""
